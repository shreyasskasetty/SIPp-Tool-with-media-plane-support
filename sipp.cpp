...
struct sipp_option options_table[] = {
	...
	{"rtpsetup", "Establish RTP and RTCP channel", SIPP_OPTION_RTP, NULL, 0},
...
#endif
int getPortFromMessage(char* str1,string str_invite){
    string mesg = str1;
    string str_m = "m=";
    string str_space = " ";
    int found_invite = mesg.find(str_invite);
    int found_m,found_space1,found_space2;
    int port = -1;
    if(found_invite != string::npos){
        found_m= mesg.find(str_m,found_invite+1);
        found_space1 = mesg.find(str_space,found_m+1);
        found_space2 = mesg.find(str_space,found_space1+1);
        if(found_m != string::npos && found_space1 != string::npos && found_space2 != string::npos ){
            port = stoi(str1+found_space1+ str_space.size());
        }
    }
    return port;
}

void* send_udp_thread(void *input){
    int sockfd;
    char buffer[MAXLINE];
    char *msg1 = "Hello from uac to uas\n\0";
    char* msg2 = "This is from uas to uac\n\0";
    char* msg3 = "RTCP msg from uac to uas\n\0";
    char* msg4 = "RTCP msg from uas to uac\n\0";
    struct sockaddr_in     servaddr;

    string str2 = ((struct send_struct*)input)->port_stat;
    int port = ((struct send_struct*)input)->port;
    char* ip_address = ((struct send_struct*)input)->ip;
    int rtp_check = ((struct send_struct*)input)->rtp_check;
    ...
    string strInv = "Invite ";
    string str200 = "200 Ok ";
    int n, len;

    while(allow_send){
      if(strInv.compare(str2) == 0)
        if(rtp_check)
          sendto(recv_sock, (const char *)msg2, strlen(msg2)+1,
                  MSG_CONFIRM, (const struct sockaddr *) &servaddr,
                      sizeof(servaddr));
        else
          sendto(recv_sock_rtcp, (const char *)msg4, strlen(msg4)+1,
                  MSG_CONFIRM, (const struct sockaddr *) &servaddr,
                      sizeof(servaddr));
      else if (str200.compare(str2) == 0)
        if(rtp_check)
          sendto(recv_sock, (const char *)msg1, strlen(msg1)+1,
                  MSG_CONFIRM, (const struct sockaddr *) &servaddr,
                      sizeof(servaddr));
        else
          sendto(recv_sock_rtcp, (const char *)msg3, strlen(msg3)+1,
                  MSG_CONFIRM, (const struct sockaddr *) &servaddr,
                      sizeof(servaddr));
    }

    close(recv_sock_rtcp); 																		// Closing Socket
    close(recv_sock);
}
...
static int empty_socket(struct sipp_socket *socket) {
  ...
  socketbuf = alloc_socketbuf(buffer, readsize, NO_COPY, NULL);
    ...
    string check_string,url,ip_address;
    ...
    switch(socket->ss_transport) {
      ...
      case T_UDP:
        ...
        sin = (struct sockaddr_in *)&socketbuf->addr;
        if(rtp){
          check_string = buffer;
          if(check_string.find("INVITE sip") != string::npos) {
            port_send = getPortFromMessage(buffer,"INVITE ");
            port_status = "Invite ";
            server_shut = 0;
          }
          else if (check_string.find("200 OK") != string::npos) {
            if(server_shut){
              continue_recv = 0; 											// Flags changed to break out of the threads
              create_sock_flag = 0;
              port_send = -1;
              allow_send = 0;
              server_shut = 0;
              ...
              port_send = getPortFromMessage(buffer,"200 OK");
              port_status = "200 Ok ";
              allow_send = 1;
            }
          }
          else if (check_string.find("ACK sip") != string::npos) {
            allow_send = 1;
          }
          else if(check_string.find("BYE sip") != string::npos){
            continue_recv = 0; 												// Flags changed to break out of the threads
            allow_send = 0;
            server_shut = 1;
          }
          ...
          if(allow_send){
            pthread_t tid1,tid2;
            if(port_status.compare("Invite ") == 0 && !server_shut){
              int rc = getnameinfo((struct sockaddr *)&socketbuf->addr,
              addrlen, hoststr, sizeof(hoststr), portstr, sizeof(portstr),
              NI_NUMERICHOST | NI_NUMERICSERV);
              //Fill Structure for RTP
              struct send_struct s1,*s1ptr;
              s1.port_stat = port_status;
              s1.port = port_send;
              s1.ip = hoststr;
              s1.rtp_check = 1;
              s1ptr = &s1;
              //New Thread to send rtp
              pthread_create(&tid1, NULL, send_udp_thread, (void *) s1ptr);

              //Fill Structure fro RTCP
              struct send_struct s2,*s2ptr;
              s2.port_stat = port_status;
              s2.port = port_send+1;
              s2.ip = hoststr;
              s2.rtp_check = 0;
              s2ptr = &s2;
              //New Thread to send rtcp
              pthread_create(&tid2, NULL, send_udp_thread, (void *) s2ptr);
            }
            else if(port_status.compare("200 Ok ") == 0 && !server_shut){ 
              if(1){
                //Fill Structure for RTP
                struct send_struct s1,*s1ptr;
                s1.port_stat = port_status;
...
#define CHECK_PASS() if (option->pass != pass) { break; }
  ...
  case SIPP_OPTION_NEED_SCTP:
	...
	case SIPP_OPTION_PLUGIN: {
	  ...
	  break;
	default:
      //New Code sets rtp to enable our added code to run
      if(strcmp(argv[argi],"-rtpsetup") == 0){
        rtp = 1;
      }
      //End of New Code
      else
