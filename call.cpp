...
// static int create_sock_flag=0,y=0;
static int break_loop = 0;
static int break_loop_rtcp = 0;
// int port_recv=-1;
...
//New Code
//Message Modifier
string getNewMessage ( char* str1, int port,string str2){
    string str3 = "m=";
    string str4 = "a=";
    string strspace = " ";
    string mainStr = str1;
    int found_invite = mainStr.find(str2);
    int found_m = 0;
    int found_space1 = 0;
    int found_space2 = 0;
    int found_a = 0;
    ...
    char templine[20];
    sprintf(templine," %d ",port);
    if(found_invite != string::npos){
        found_m=mainStr.find(str3);
        found_space1 = mainStr.find(strspace,found_m+1);
        found_space2 = mainStr.find(strspace,found_space1+1);
        if(found_space1 != string::npos && found_space2 != string::npos && found_m!= string::npos){
            temp1 = mainStr.substr(0,found_space1);
            temp3 = mainStr.substr(found_space2+1,mainStr.size());
            temp2 = templine;
            mainStr = temp1 + temp2 + temp3;
        }
    }
    char  returnTemp[strlen(mainStr.c_str())];
    strcpy(returnTemp,mainStr.c_str());
    return returnTemp;
}

//End of New Code Message Modifier
//New Code Socket Function
int create_socket(int choice,int set_port){
  int sockfd;
  int opt = 1;
  int loop = 200; 																	//Check for 100 even ports
  int PORT = 8000+call_count;
  int i = 0,flag=1;
  struct sockaddr_in servaddr;
  if(choice) 																		// Setup UAC and UAS ports separately
    PORT = PORT + 100;
  /**if(set_port)
    PORT = set_port;**/
  ...
  while(loop > i) {
    if ( (sockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0 ) {  						// Resource Exhaustion happens here
            perror("socket creation failed call.cpp");
            exit(EXIT_FAILURE);
        }
        /**/
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,&opt, sizeof(opt)))    //Allows reuse of ports
    {
        perror("setsockopt");
        exit(EXIT_FAILURE);
    }
...
int get_port(int sockfd){
  ...
  if (getsockname(sockfd, (struct sockaddr *)&temp_addr, &len) == -1) {
      perror("getsockname");
      return -1;
  }
...
void *recv_udp_thread(void *arg) {
  ...
  break_loop = 1;
  while(continue_recv){
        n = recvfrom(recv_sock, (char *)buffer, MAXLINE,
                MSG_WAITALL, ( struct sockaddr *) &cliaddr,
                &len);
        buffer[n-1] = '\0';
    }

  close(recv_sock); 													// Closing socket
  return NULL;
}
void *recv_rtcp_thread(void *arg) {
  int n;
  char buffer[MAXLINE];
  socklen_t len;
  sockaddr_in cliaddr;
  break_loop_rtcp = 1;
  while(continue_recv){
        n = recvfrom(recv_sock_rtcp, (char *)buffer, MAXLINE,
                MSG_WAITALL, ( struct sockaddr *) &cliaddr,
                &len);
        buffer[n-1] = '\0';
    }
  close(recv_sock_rtcp);
  return NULL;
}
//End of New Code
int call::send_raw(char * msg, int index, int len)
{
  //New Code
  if(rtp){
    // Modifying the msg variable with our new message
    string temp_string;
    string check_string;
    int sockfd;
    size_t templen;
    pthread_t tid1,tid2;
        ...
        if(check_string.find("INVITE sip") != string::npos) {
          if(create_sock_flag == 0){
            create_sock_flag = 1;
            server_shut = 0;
            recv_sock = create_socket(1,0);
            port_recv = get_port(recv_sock);
            recv_sock_rtcp = create_socket(1,port_recv+1);
          }
          temp_string = getNewMessage(msg,port_recv,"INVITE "); 			// arguments msg, portno, msg type
          strcpy(msg,temp_string.c_str());
          create_sock_flag = 1;
        }
          ...
          if(server_shut){
            ...
            if(create_sock_flag == 0){
              create_sock_flag = 1;
              recv_sock = create_socket(0,0);
              port_recv = get_port(recv_sock);
              continue_recv = 1;
              pthread_create(&tid1, NULL, recv_udp_thread, NULL);
              //RTCP Code
              recv_sock_rtcp = create_socket(0,port_recv+1);
              pthread_create(&tid2, NULL, recv_rtcp_thread, NULL);
              //End of RTCP Code
            }
            temp_string = getNewMessage(msg,port_recv,"200 OK");
            strcpy(msg,temp_string.c_str());
            create_sock_flag = 1;
          }
        }
